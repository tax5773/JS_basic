let app = {
    ...
    generateRow(startWithColor, rowNum) {
        ...
        for (let i = 0; i < this.config.rows.length; i++) {
            ...
            if (currentColorClass === 'white') {
                ...
                field = this.generateField(currentColorClass, rowNum , this.config.cols[i]); // !!!!!!!!! rowNum подсмотрел в конце потому что никак не получалось. Знаю что тупанул.
    ...
    insertColsChars() {
        ...
        let tbody = document.querySelector('tbody'); // !!!! написал просто body подсмотрел чтобы исправить. Почему tbody? Помню что чтото обьяснялось, но в 2х словах пжл
