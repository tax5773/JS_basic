...
function insertPosts() {
    ...
    // !!! тут не выдержал и подсмотрел. изначально условия были for (let i = 1; i < 0; i++);
    // не понимал почему не работает. Теперь кажется дошло
    // нам нужно создать колличество постов достаточное чтобы они просто ушли за грань видимости
    // и только когда начинается scrollCheck, сново используется генерация постов чтобы ушли за грань
    // экрана. А когда for (let i = 1; i < 0; i++); то insertPosts делает посты пока не наклепает все
    // соответственно Chrome просто виснет в этом бесконечном цикле. Верно я понял?
    //
    // Все остальное удалось написать правильно с первого раза.
