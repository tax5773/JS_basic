...
//немного смухлевал в этом задании. подсматривал.(((
//этот блок написал сам.
...
//тут тоже разобрался. благодаря прошлым урокам
...
// Вот тут сломался мозг... (((
function showCategory(category) {
    const categoryProducts = products[category]; // Никак не мог
    let insertProductsInToDiv = '';              // врубиться что тут нужно
    categoryProducts.forEach(function(elem){     //  дополнительный обьект сделать с элементами в конкремной категорий
        insertProductsInToDiv += getProductMarkup(elem);
    })
    productsWrap.insertAdjacentHTML('afterbegin', insertProductsInToDiv); // Была в начале только эта строка
}
